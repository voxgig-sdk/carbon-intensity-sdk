...
describe("GenerationDirect", function()
  it("should direct-list-generation", function()
    local setup = generation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-generation", setup.live and "live" or "unit")
  ...
  it("should direct-load-generation", function()
    local setup = generation_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-generation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function generation_direct_setup(mockres)
