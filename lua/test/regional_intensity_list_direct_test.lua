-- RegionalIntensityList direct test
...
describe("RegionalIntensityListDirect", function()
  it("should direct-list-regional_intensity_list", function()
    local setup = regional_intensity_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-regional_intensity_list", setup.live and "live" or "unit")
  ...
  it("should direct-load-regional_intensity_list", function()
    local setup = regional_intensity_list_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-regional_intensity_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function regional_intensity_list_direct_setup(mockres)
