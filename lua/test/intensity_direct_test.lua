...
describe("IntensityDirect", function()
  it("should direct-list-intensity", function()
    local setup = intensity_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-intensity", setup.live and "live" or "unit")
  ...
  it("should direct-load-intensity", function()
    local setup = intensity_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-intensity", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function intensity_direct_setup(mockres)
