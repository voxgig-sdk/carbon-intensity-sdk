...
describe("StatEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Stat(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Stat(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_STAT_ENTID JSON to run live")
      return
    end
    ...
    local stat_ref01_match = {
      ["from"] = setup.idmap["from01"],
      ["to"] = setup.idmap["to01"],
    }

    local stat_ref01_list_result, err = stat_ref01_ent:list(stat_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(stat_ref01_list_result)
...
function stat_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CARBON_INTENSITY_TEST_STAT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CARBON_INTENSITY_TEST_STAT_ENTID"])
