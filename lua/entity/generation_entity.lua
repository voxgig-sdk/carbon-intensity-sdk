...
local GenerationEntity = {}
GenerationEntity.__index = GenerationEntity


function GenerationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GenerationEntity)
...
function GenerationEntity:get_name()
  return self._name
end


function GenerationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GenerationEntity.new(self._client, opts)
end
...
function GenerationEntity:mark_deleted()
  self._deleted = true
end


function GenerationEntity:deleted()
  return true == self._deleted
end


function GenerationEntity:data_set(args)
  if args ~= nil then
...
function GenerationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GenerationEntity:match_set(args)
  if args ~= nil then
...
function GenerationEntity:match_get()
...
function GenerationEntity:stream(action, args, callopts)
...
---@param reqmatch GenerationLoadMatch
---@param ctrl? table
---@return Generation
---@return string? err
function GenerationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GenerationEntity:_run_op(ctx, post_done)
  local utility = self._utility
