...
local StatEntity = {}
StatEntity.__index = StatEntity


function StatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StatEntity)
  self._name = "stat"
  self._client = client
...
function StatEntity:get_name()
  return self._name
end


function StatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StatEntity.new(self._client, opts)
end
...
function StatEntity:mark_deleted()
  self._deleted = true
end


function StatEntity:deleted()
  return true == self._deleted
end


function StatEntity:data_set(args)
  if args ~= nil then
...
function StatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StatEntity:match_set(args)
  if args ~= nil then
...
function StatEntity:stream(action, args, callopts)
...
function StatEntity:_run_op(ctx, post_done)
  local utility = self._utility
