-- CarbonIntensity SDK IntensityFactor entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local IntensityFactorEntity = {}
IntensityFactorEntity.__index = IntensityFactorEntity


function IntensityFactorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IntensityFactorEntity)
  self._name = "intensity_factor"
  self._client = client
...
function IntensityFactorEntity:get_name()
  return self._name
end


function IntensityFactorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IntensityFactorEntity.new(self._client, opts)
end
...
function IntensityFactorEntity:mark_deleted()
  self._deleted = true
end


function IntensityFactorEntity:deleted()
  return true == self._deleted
end


function IntensityFactorEntity:data_set(args)
  if args ~= nil then
...
function IntensityFactorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IntensityFactorEntity:match_set(args)
  if args ~= nil then
...
function IntensityFactorEntity:match_get()
...
function IntensityFactorEntity:stream(action, args, callopts)
...
---@param reqmatch IntensityFactorListMatch
---@param ctrl? table
---@return IntensityFactor[]
---@return string? err
function IntensityFactorEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function IntensityFactorEntity:_run_op(ctx, post_done)
  local utility = self._utility
