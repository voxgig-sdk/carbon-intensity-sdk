-- CarbonIntensity SDK GenerationList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GenerationListEntity = {}
GenerationListEntity.__index = GenerationListEntity


function GenerationListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GenerationListEntity)
  self._name = "generation_list"
  self._client = client
...
function GenerationListEntity:get_name()
  return self._name
end


function GenerationListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GenerationListEntity.new(self._client, opts)
end
...
function GenerationListEntity:mark_deleted()
  self._deleted = true
end


function GenerationListEntity:deleted()
  return true == self._deleted
end


function GenerationListEntity:data_set(args)
  if args ~= nil then
...
function GenerationListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GenerationListEntity:match_set(args)
  if args ~= nil then
...
function GenerationListEntity:match_get()
...
function GenerationListEntity:stream(action, args, callopts)
...
---@param reqmatch GenerationListListMatch
---@param ctrl? table
---@return GenerationList[]
---@return string? err
function GenerationListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GenerationListEntity:_run_op(ctx, post_done)
  local utility = self._utility
