-- CarbonIntensity SDK Regional entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RegionalEntity = {}
RegionalEntity.__index = RegionalEntity


function RegionalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RegionalEntity)
  self._name = "regional"
  self._client = client
...
function RegionalEntity:get_name()
  return self._name
end


function RegionalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RegionalEntity.new(self._client, opts)
end
...
function RegionalEntity:mark_deleted()
  self._deleted = true
end


function RegionalEntity:deleted()
  return true == self._deleted
end


function RegionalEntity:data_set(args)
  if args ~= nil then
...
function RegionalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RegionalEntity:match_set(args)
  if args ~= nil then
...
function RegionalEntity:match_get()
...
function RegionalEntity:stream(action, args, callopts)
...
---@param reqmatch RegionalListMatch
---@param ctrl? table
---@return Regional[]
---@return string? err
function RegionalEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RegionalEntity:_run_op(ctx, post_done)
  local utility = self._utility
