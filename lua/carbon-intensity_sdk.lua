...
require("carbon-intensity_types")
...
local CarbonIntensitySDK = {}
CarbonIntensitySDK.__index = CarbonIntensitySDK
...
CarbonIntensitySDK._make_feature = _make_feature


function CarbonIntensitySDK.new(options)
  local self = setmetatable({}, CarbonIntensitySDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CarbonIntensitySDK:options_map()
...
function CarbonIntensitySDK:get_utility()
  return Utility.copy(self._utility)
end


function CarbonIntensitySDK:get_root_ctx()
  return self._rootctx
end


function CarbonIntensitySDK:prepare(fetchargs)
...
function CarbonIntensitySDK:direct(fetchargs)
...
function CarbonIntensitySDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CarbonIntensitySDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CarbonIntensitySDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CarbonIntensitySDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CarbonIntensitySDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CarbonIntensitySDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CarbonIntensitySDK:GenerationList(data)
  ...
  if data == nil then
    if self._generation_list == nil then
      self._generation_list = EntityMod.new(self, nil)
    end
...
function CarbonIntensitySDK:Intensity(data)
  ...
  if data == nil then
    if self._intensity == nil then
      self._intensity = EntityMod.new(self, nil)
    end
...
function CarbonIntensitySDK:IntensityFactor(data)
  ...
  if data == nil then
    if self._intensity_factor == nil then
      self._intensity_factor = EntityMod.new(self, nil)
    end
...
function CarbonIntensitySDK:IntensityList(data)
  ...
  if data == nil then
    if self._intensity_list == nil then
      self._intensity_list = EntityMod.new(self, nil)
    end
...
function CarbonIntensitySDK:Regional(data)
  ...
  if data == nil then
    if self._regional == nil then
      self._regional = EntityMod.new(self, nil)
    end
...
function CarbonIntensitySDK:RegionalIntensity(data)
  ...
  if data == nil then
    if self._regional_intensity == nil then
      self._regional_intensity = EntityMod.new(self, nil)
    end
    return self._regional_intensity
  end
  return EntityMod.new(self, data)
end
...
function CarbonIntensitySDK:RegionalIntensityList(data)
  ...
  if data == nil then
    if self._regional_intensity_list == nil then
      self._regional_intensity_list = EntityMod.new(self, nil)
    end
    return self._regional_intensity_list
  end
  return EntityMod.new(self, data)
end
...
function CarbonIntensitySDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CarbonIntensitySDK.new(sdkopts)
  sdk.mode = "test"
