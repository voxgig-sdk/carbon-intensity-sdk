# CarbonIntensity SDK utility: make_request
...
module CarbonIntensityUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CarbonIntensityResponse.new({})
    ...
    if fetch_err
      ...
      response = CarbonIntensityResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CarbonIntensityResponse.new(fetched)
    else
