...
// CarbonIntensity SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CarbonIntensityTestFeature extends CarbonIntensityBaseFeature
{
    ...
    public function init(CarbonIntensityContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CarbonIntensityContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CarbonIntensityParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CarbonIntensityContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CarbonIntensityContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CarbonIntensityParam::call($ctx, $k);
