...
// CarbonIntensity SDK pipeline test
//
...
{
    private static function utility(): CarbonIntensityUtility
    {
        return new CarbonIntensityUtility();
    }
    ...
    private static function ctx(array $over = []): CarbonIntensityContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CarbonIntensityContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CarbonIntensityOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CarbonIntensityError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CarbonIntensityOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CarbonIntensityMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CarbonIntensityOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CarbonIntensityMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CarbonIntensityOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CarbonIntensityMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CarbonIntensitySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CarbonIntensityMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new CarbonIntensityResult([]),
        ]);
        [, $err] = CarbonIntensityMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CarbonIntensitySpec([]),
            'response' => null,
            'result' => new CarbonIntensityResult([]),
        ]);
        [, $err] = CarbonIntensityMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CarbonIntensitySpec(['step' => 's']),
            'response' => new CarbonIntensityResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CarbonIntensityResult([]),
        ]);
        [, $err] = CarbonIntensityMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CarbonIntensitySpec(['step' => 's']),
            'response' => new CarbonIntensityResponse(self::resp(200, ['v' => 1])),
            'result' => new CarbonIntensityResult([]),
        ]);
        [, $err] = CarbonIntensityMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CarbonIntensitySpec(['step' => 's']),
            'response' => new CarbonIntensityResponse(self::resp(200, ['v' => 2])),
            'result' => new CarbonIntensityResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CarbonIntensityMakeResponse::call($ctx);
    ...
    {
        $preset = new CarbonIntensityResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CarbonIntensitySpec([]),
            'response' => new CarbonIntensityResponse([]),
            'result' => new CarbonIntensityResult([]),
        ...
        [$got, $err] = CarbonIntensityMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CarbonIntensityResult([])]);
        [, $err] = CarbonIntensityMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CarbonIntensitySpec([]), 'result' => null]);
        [, $err] = CarbonIntensityMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CarbonIntensitySpec(['step' => 's']),
            'result' => new CarbonIntensityResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CarbonIntensityOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CarbonIntensityMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CarbonIntensitySpec(['step' => 's']),
            'result' => new CarbonIntensityResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CarbonIntensityOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CarbonIntensityMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CarbonIntensityResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CarbonIntensitySpec([]),
            'result' => new CarbonIntensityResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CarbonIntensityMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CarbonIntensityMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CarbonIntensityError('boom', 'boom');
        $utility->fetcher = function (CarbonIntensityContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CarbonIntensitySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CarbonIntensityMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CarbonIntensityContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CarbonIntensityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CarbonIntensityResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CarbonIntensityMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CarbonIntensityContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CarbonIntensitySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CarbonIntensityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CarbonIntensityResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CarbonIntensitySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CarbonIntensityMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CarbonIntensityMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CarbonIntensityMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CarbonIntensityResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CarbonIntensityDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CarbonIntensityResult(['ok' => false])]);
        $this->expectException(CarbonIntensityError::class);
        CarbonIntensityDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CarbonIntensityResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CarbonIntensityMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CarbonIntensityMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CarbonIntensityBaseFeature();
        $b = new CarbonIntensityBaseFeature();
        CarbonIntensityFeatureAdd::call($ctx, $a);
        CarbonIntensityFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CarbonIntensityBaseFeature
    {
        $f = new CarbonIntensityBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CarbonIntensityFeatureAdd::call($ctx, self::named_feature('a'));
        CarbonIntensityFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CarbonIntensityFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CarbonIntensityFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CarbonIntensityFeatureAdd::call($ctx, $replace);
        ...
        CarbonIntensityFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CarbonIntensityContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CarbonIntensitySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CarbonIntensityPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CarbonIntensityPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CarbonIntensityResponse(['headers' => null]),
            'result' => new CarbonIntensityResult([]),
        ]);
        CarbonIntensityResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CarbonIntensityResultBody::call($ctx);
