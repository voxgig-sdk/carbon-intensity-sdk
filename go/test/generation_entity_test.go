...
func TestGenerationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Generation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Generation(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_GENERATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		generationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.generation")))
		...
		generationRef01MatchDt0 := map[string]any{
			"id": generationRef01Data["id"],
		}
		generationRef01DataDt0Loaded, err := generationRef01Ent.Load(generationRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		generationRef01DataDt0LoadResult := core.ToMapAny(entityData(generationRef01DataDt0Loaded))
		if generationRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if generationRef01DataDt0LoadResult["id"] != generationRef01Data["id"] {
...
func generationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CARBON_INTENSITY_TEST_GENERATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CARBON_INTENSITY_TEST_GENERATION_ENTID"])
