...
func TestIntensityDirect(t *testing.T) {
	t.Run("direct-list-intensity", func(t *testing.T) {
		setup := intensityDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-intensity", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-intensity", func(t *testing.T) {
		setup := intensityDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-intensity", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type intensityDirectSetupResult struct {
...
func intensityDirectSetup(mockres any) *intensityDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CARBON_INTENSITY_TEST_INTENSITY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CARBON_INTENSITY_TEST_INTENSITY_ENTID"]; ok {
		...
		return &intensityDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &intensityDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
