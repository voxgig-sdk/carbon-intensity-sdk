...
func TestIntensityListEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.IntensityList(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.IntensityList(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		intensityListRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.intensity_list")))
...
func intensity_listBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID"])
