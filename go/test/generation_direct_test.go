...
func TestGenerationDirect(t *testing.T) {
	t.Run("direct-list-generation", func(t *testing.T) {
		setup := generationDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-generation", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-generation", func(t *testing.T) {
		setup := generationDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-generation", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type generationDirectSetupResult struct {
...
func generationDirectSetup(mockres any) *generationDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CARBON_INTENSITY_TEST_GENERATION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CARBON_INTENSITY_TEST_GENERATION_ENTID"]; ok {
		...
		return &generationDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &generationDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
