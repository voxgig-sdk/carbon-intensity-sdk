...
func TestIntensityListDirect(t *testing.T) {
	t.Run("direct-list-intensity_list", func(t *testing.T) {
		setup := intensity_listDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-intensity_list", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-intensity_list", func(t *testing.T) {
		setup := intensity_listDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-intensity_list", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type intensity_listDirectSetupResult struct {
...
func intensity_listDirectSetup(mockres any) *intensity_listDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID"]; ok {
		...
		return &intensity_listDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &intensity_listDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
