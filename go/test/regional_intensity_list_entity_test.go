...
func TestRegionalIntensityListEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		regionalIntensityListRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.regional_intensity_list")))
		...
		regionalIntensityListRef01MatchDt0 := map[string]any{
			"id": regionalIntensityListRef01Data["id"],
		}
		...
		regionalIntensityListRef01DataDt0LoadResult := core.ToMapAny(entityData(regionalIntensityListRef01DataDt0Loaded))
		if regionalIntensityListRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if regionalIntensityListRef01DataDt0LoadResult["id"] != regionalIntensityListRef01Data["id"] {
...
func regional_intensity_listBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_LIST_ENTID"])
