...
func TestStatDirect(t *testing.T) {
	t.Run("direct-load-stat", func(t *testing.T) {
		setup := statDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-stat", _mode); _shouldSkip {
			if _reason == "" {
