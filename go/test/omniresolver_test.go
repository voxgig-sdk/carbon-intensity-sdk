...
import (
	...
	omni "github.com/voxgig-sdk/carbon-intensity-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CarbonIntensityError); is {
				out := map[string]any{
					"name":    "CarbonIntensityError",
...
func omniCtx(arg any, client *sdk.CarbonIntensitySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CarbonIntensitySDK); is {
			client = live
