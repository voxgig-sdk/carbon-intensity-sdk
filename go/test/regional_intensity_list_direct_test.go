...
func TestRegionalIntensityListDirect(t *testing.T) {
	t.Run("direct-list-regional_intensity_list", func(t *testing.T) {
		setup := regional_intensity_listDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-regional_intensity_list", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-regional_intensity_list", func(t *testing.T) {
		setup := regional_intensity_listDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-regional_intensity_list", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type regional_intensity_listDirectSetupResult struct {
	client *sdk.CarbonIntensitySDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func regional_intensity_listDirectSetup(mockres any) *regional_intensity_listDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &regional_intensity_listDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &regional_intensity_listDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
