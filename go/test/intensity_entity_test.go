...
func TestIntensityEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Intensity(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Intensity(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		intensityRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.intensity")))
...
func intensityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CARBON_INTENSITY_TEST_INTENSITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CARBON_INTENSITY_TEST_INTENSITY_ENTID"])
