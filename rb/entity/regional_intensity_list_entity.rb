...
class RegionalIntensityListEntity
  ...
  def make
    opts = @_entopts.dup
    RegionalIntensityListEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CarbonIntensityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RegionalIntensityList, Hash] the current RegionalIntensityList data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CarbonIntensityHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RegionalIntensityList fields)
  def match_get
  ...
  #
  # @param reqmatch [RegionalIntensityListLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RegionalIntensityList.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RegionalIntensityList, Hash] the loaded RegionalIntensityList; raises CarbonIntensityError on failure
  ...
  #
  # @param reqmatch [RegionalIntensityListListMatch, Hash, nil] match filter (any subset of
  #   RegionalIntensityList fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<RegionalIntensityList>, Array] the matching RegionalIntensityList items; raises CarbonIntensityError on failure
