...
class RegionalIntensityEntity
  ...
  def make
    opts = @_entopts.dup
    RegionalIntensityEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.RegionalIntensity.load works with no args).
  ...
  #
  # @param reqmatch [RegionalIntensityListMatch, Hash, nil] match filter (any subset of
  #   RegionalIntensity fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
