...
class CarbonIntensitySDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CarbonIntensityHelpers.to_map(feature_opts[fname])
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CarbonIntensityError.new(
        "graphql_error", "CarbonIntensitySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
