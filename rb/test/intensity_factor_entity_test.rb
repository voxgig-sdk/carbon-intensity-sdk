...
class IntensityFactorEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.IntensityFactor(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.IntensityFactor(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_INTENSITY_FACTOR_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def intensity_factor_basic_setup(extra)
  ...
  entid_env_raw = ENV["CARBON_INTENSITY_TEST_INTENSITY_FACTOR_ENTID"]
