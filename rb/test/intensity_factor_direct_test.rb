...
class IntensityFactorDirectTest < Minitest::Test
  def test_direct_list_intensity_factor
    setup = intensity_factor_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-intensity_factor", setup[:live] ? "live" : "unit")
    if _should_skip
...
def intensity_factor_direct_setup(mockres)
