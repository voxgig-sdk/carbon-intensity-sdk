...
class StatDirectTest < Minitest::Test
  def test_direct_list_stat
    setup = stat_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-stat", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["block01", "from01", "to01"].each do |_live_key|
