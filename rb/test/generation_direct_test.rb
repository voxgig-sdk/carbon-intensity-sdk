...
class GenerationDirectTest < Minitest::Test
  def test_direct_list_generation
    setup = generation_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-generation", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_generation
    setup = generation_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-generation", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def generation_direct_setup(mockres)
