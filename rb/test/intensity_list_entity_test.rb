...
class IntensityListEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.IntensityList(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.IntensityList(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    intensity_list_ref01_list_result = intensity_list_ref01_ent.list(intensity_list_ref01_match, nil)
    assert intensity_list_ref01_list_result.is_a?(Array)

    # LOAD
    intensity_list_ref01_match_dt0 = {}
    intensity_list_ref01_data_dt0_loaded = intensity_list_ref01_ent.load(intensity_list_ref01_match_dt0, nil)
...
def intensity_list_basic_setup(extra)
  ...
  entid_env_raw = ENV["CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID"]
