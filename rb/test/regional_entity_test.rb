...
class RegionalEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Regional(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Regional(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = regional_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "regional." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    regional_ref01_data = nil
    if regional_ref01_data_raw.length > 0
...
def regional_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "regional", "RegionalTestData.json")
