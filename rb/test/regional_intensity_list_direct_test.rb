...
class RegionalIntensityListDirectTest < Minitest::Test
  def test_direct_list_regional_intensity_list
    setup = regional_intensity_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-regional_intensity_list", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_regional_intensity_list
    setup = regional_intensity_list_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-regional_intensity_list", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def regional_intensity_list_direct_setup(mockres)
