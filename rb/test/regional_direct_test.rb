...
class RegionalDirectTest < Minitest::Test
  def test_direct_list_regional
    setup = regional_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-regional", setup[:live] ? "live" : "unit")
    if _should_skip
...
def regional_direct_setup(mockres)
