...
(0, node_test_1.describe)('RegionalIntensityListDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.CarbonIntensitySDK({
    ...
    (0, node_test_1.test)('direct-load-regional_intensity_list', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-regional_intensity_list', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["intensity01", "postcode01", "to01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.intensity_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.intensity_id = candidateId;
            params.intensity_id = setup.idmap['intensity01'];
            params.postcode = setup.idmap['postcode01'];
            params.to = setup.idmap['to01'];
        }
        else {
            params.intensity_id = 'direct01';
            params.postcode = 'direct02';
            params.to = 'direct03';
        }
    ...
    (0, node_test_1.test)('direct-list-regional_intensity_list', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-regional_intensity_list', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["from01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CARBON_INTENSITY_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CarbonIntensitySDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_LIST_ENTID'];
    ...
    const client = new __1.CarbonIntensitySDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=RegionalIntensityListDirect.test.js.map