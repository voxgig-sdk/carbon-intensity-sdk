...
(0, node_test_1.describe)('RegionalIntensityDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-regional_intensity', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-regional_intensity', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["postcode01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.postcode ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.postcode = candidateId;
    ...
    (0, node_test_1.test)('direct-list-regional_intensity', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-regional_intensity', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_ENTID'];
...
//# sourceMappingURL=RegionalIntensityDirect.test.js.map