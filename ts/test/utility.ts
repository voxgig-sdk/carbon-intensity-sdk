...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.CARBONINTENSITY_TEST_LIVE ||
    'TRUE' === process.env.CARBONINTENSITY_TEST_OVERRIDE
  ) {
  ...
  m.CARBONINTENSITY_TEST_EXPLAIN = process.env.CARBONINTENSITY_TEST_EXPLAIN || m.CARBONINTENSITY_TEST_EXPLAIN
