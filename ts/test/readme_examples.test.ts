...
//     (\`new CarbonIntensitySDK(...)\` / \`CarbonIntensitySDK.test(...)\` become
//     \`CarbonIntensitySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CarbonIntensitySDK'
...
const TEST_SEED = {"entity":{"generation":{"test01":{"id":"test01"}},"generation_list":{"test01":{"id":"test01"}},"intensity":{"test01":{"id":"test01"}},"intensity_factor":{"test01":{"id":"test01"}},"intensity_list":{"test01":{"id":"test01"}},"regional":{"test01":{"id":"test01"}},"regional_intensity":{"test01":{"id":"test01"}},"regional_intensity_list":{"test01":{"id":"test01"}},"stat":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CarbonIntensitySDK(...)\` and any
// \`CarbonIntensitySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CarbonIntensitySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CarbonIntensitySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
