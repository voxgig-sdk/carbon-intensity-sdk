...
describe('IntensityListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'intensity_list.' + op, live)) return
    }
    ...
    const intensity_list_ref01_list = await intensity_list_ref01_ent.list(intensity_list_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID']
