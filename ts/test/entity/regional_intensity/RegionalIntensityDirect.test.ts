...
describe('RegionalIntensityDirect', async () => {
  ...
  test('direct-load-regional_intensity', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-regional_intensity', setup.live)) return
    if (skipIfMissingIds(t, setup, ["postcode01"])) return
  ...
  test('direct-list-regional_intensity', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-regional_intensity', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_ENTID']
