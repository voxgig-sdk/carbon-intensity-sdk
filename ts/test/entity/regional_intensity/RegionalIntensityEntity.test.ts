...
describe('RegionalIntensityEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'regional_intensity.' + op, live)) return
    }
    ...
    const regional_intensity_ref01_list = await regional_intensity_ref01_ent.list(regional_intensity_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_ENTID']
