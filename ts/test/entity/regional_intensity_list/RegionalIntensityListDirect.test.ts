...
describe('RegionalIntensityListDirect', async () => {
  ...
  test('direct-load-regional_intensity_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-regional_intensity_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["intensity01","postcode01","to01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_LIST_ENTID']
