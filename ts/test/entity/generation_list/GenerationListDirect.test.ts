...
describe('GenerationListDirect', async () => {
  ...
  test('direct-list-generation_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-generation_list', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CARBON_INTENSITY_TEST_GENERATION_LIST_ENTID']
