...
class GenerationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CarbonIntensitySDK::test(null, null);
        $ent = $testsdk->Generation(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Generation(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Generation(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_GENERATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $generation_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.generation")));
        $generation_ref01_data = null;
        if (count($generation_ref01_data_raw) > 0) {
            $generation_ref01_data = Helpers::to_map($generation_ref01_data_raw[0][1]);
        }
...
function generation_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/generation/GenerationTestData.json';
    ...
    $entid_env_raw = getenv("CARBON_INTENSITY_TEST_GENERATION_ENTID");
