...
class IntensityEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CarbonIntensitySDK::test(null, null);
        $ent = $testsdk->Intensity(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Intensity(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Intensity(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = intensity_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "intensity." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_INTENSITY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $intensity_ref01_data = null;
        if (count($intensity_ref01_data_raw) > 0) {
            $intensity_ref01_data = Helpers::to_map($intensity_ref01_data_raw[0][1]);
        }

        // LIST
        $intensity_ref01_ent = $client->Intensity(null);
        $intensity_ref01_match = [];

        $intensity_ref01_list_result = $intensity_ref01_ent->list($intensity_ref01_match, null);
...
function intensity_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/intensity/IntensityTestData.json';
    ...
    $entid_env_raw = getenv("CARBON_INTENSITY_TEST_INTENSITY_ENTID");
