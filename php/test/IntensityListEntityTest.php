...
class IntensityListEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CarbonIntensitySDK::test(null, null);
        $ent = $testsdk->IntensityList(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->IntensityList(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->IntensityList(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = intensity_list_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "intensity_list." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $intensity_list_ref01_data = null;
        if (count($intensity_list_ref01_data_raw) > 0) {
...
function intensity_list_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/intensity_list/IntensityListTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["intensity_list01", "intensity_list02", "intensity_list03", "date01", "date02", "date03", "intensity01", "intensity02", "intensity03"] as $k) {
    ...
    $entid_env_raw = getenv("CARBON_INTENSITY_TEST_INTENSITY_LIST_ENTID");
