...
// CarbonIntensity SDK feature test
//
...
class FtCtrl extends CarbonIntensityControl
{
...
class FtClient
{
    ...
    private CarbonIntensityUtility $utility;

    public function __construct(CarbonIntensityUtility $utility, array $options)
    {
    ...
    public function get_utility(): CarbonIntensityUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CarbonIntensityContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CarbonIntensityContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CarbonIntensityContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CarbonIntensityConfig::make_feature($name);
    ...
    private static function buildUrl(CarbonIntensitySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CarbonIntensityError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CarbonIntensityResponse($fetched);
        }
    ...
    private function fail(CarbonIntensityContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CarbonIntensityContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
