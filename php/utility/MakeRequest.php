...
class CarbonIntensityMakeRequest
{
    public static function call(CarbonIntensityContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new CarbonIntensityResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new CarbonIntensityResponse($fetched);
        } else {
