...
// CarbonIntensity SDK retry feature
...
class CarbonIntensityRetryFeature extends CarbonIntensityBaseFeature
{
    ...
    public function init(CarbonIntensityContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CarbonIntensityContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CarbonIntensityContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CarbonIntensityContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
