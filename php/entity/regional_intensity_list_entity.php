...
// CarbonIntensity SDK RegionalIntensityList entity
...
class RegionalIntensityListEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "regional_intensity_list";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new RegionalIntensityListEntity($this->_client, $opts);
    }

    /**
     * @param RegionalIntensityList|array $args RegionalIntensityList data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CarbonIntensityHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RegionalIntensityList|array The current RegionalIntensityList data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RegionalIntensityList fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CarbonIntensityHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RegionalIntensityList fields).
     */
    ...
    /**
     * Load a single RegionalIntensityList.
     *
     * @param RegionalIntensityListLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RegionalIntensityListLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RegionalIntensityList|array The loaded RegionalIntensityList as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CarbonIntensityHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List RegionalIntensityList items matching the given filter.
     *
     * @param RegionalIntensityListListMatch|array|null $reqmatch Match filter (any subset
     *   of RegionalIntensityList fields) as an assoc-array; RegionalIntensityListListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RegionalIntensityList[]|array A list of RegionalIntensityList items as assoc-arrays at
