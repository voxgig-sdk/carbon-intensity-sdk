...
from carbonintensity_sdk.core.helpers import get_ctx_prop, to_map
...
class CarbonIntensityContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = CarbonIntensityControl()
        ...
        if isinstance(sp, CarbonIntensitySpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CarbonIntensityResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CarbonIntensityResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return CarbonIntensityOperation({})
    ...
    def make_error(self, code, msg):
        return CarbonIntensityError(code, msg, self)
