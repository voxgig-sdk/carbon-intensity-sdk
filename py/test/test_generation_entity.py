...
class TestGenerationEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Generation(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Generation(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _generation_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CARBON_INTENSITY_TEST_GENERATION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        generation_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.generation")))
        generation_ref01_data = None
        if len(generation_ref01_data_raw) > 0:
...
def _generation_basic_setup(extra):
    ...
    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/generation/GenerationTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CARBON_INTENSITY_TEST_GENERATION_ENTID"))
