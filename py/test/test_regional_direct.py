...
class TestRegionalDirect:

    def test_should_direct_list_regional(self):
        setup = _regional_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-regional", "live" if setup["live"] else "unit")
        if _skip:
...
def _regional_direct_setup(mockres):
