...
class TestStatDirect:
    ...
    def test_should_direct_load_stat(self):
        setup = _stat_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-stat", "live" if setup["live"] else "unit")
        if _skip:
