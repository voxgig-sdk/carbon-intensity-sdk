...
class TestIntensityFactorDirect:

    def test_should_direct_list_intensity_factor(self):
        setup = _intensity_factor_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-intensity_factor", "live" if setup["live"] else "unit")
        if _skip:
...
def _intensity_factor_direct_setup(mockres):
