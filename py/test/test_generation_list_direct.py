...
class TestGenerationListDirect:

    def test_should_direct_list_generation_list(self):
        setup = _generation_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-generation_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _generation_list_direct_setup(mockres):
