...
class TestRegionalIntensityListEntity:

    def test_should_create_instance(self):
        testsdk = CarbonIntensitySDK.test(None, None)
        ent = testsdk.RegionalIntensityList(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.RegionalIntensityList(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.RegionalIntensityList(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _regional_intensity_list_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "regional_intensity_list." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_LIST_ENTID JSON to run live")
        client = setup["client"]
        ...
        regional_intensity_list_ref01_data = None
        if len(regional_intensity_list_ref01_data_raw) > 0:
            regional_intensity_list_ref01_data = helpers.to_map(regional_intensity_list_ref01_data_raw[0][1])

        # LIST
        regional_intensity_list_ref01_ent = client.RegionalIntensityList(None)
...
def _regional_intensity_list_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/regional_intensity_list/RegionalIntensityListTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CARBON_INTENSITY_TEST_REGIONAL_INTENSITY_LIST_ENTID"))
