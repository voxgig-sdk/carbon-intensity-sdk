# CarbonIntensity SDK test runner
...
class CarbonIntensityTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CarbonIntensityTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CarbonIntensityTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CarbonIntensityTestRunner.getenv("CARBON_INTENSITY_TEST_LIVE")
        override = CarbonIntensityTestRunner.getenv("CARBON_INTENSITY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CarbonIntensityTestRunner.getenv(key)
        ...
        explain = CarbonIntensityTestRunner.getenv("CARBON_INTENSITY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CarbonIntensityTestRunner._test_control is not None:
            return CarbonIntensityTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CarbonIntensityTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CarbonIntensityTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CarbonIntensityTestRunner.load_test_control()
...
def load_env_local():
    CarbonIntensityTestRunner.load_env_local()


def env_override(m):
    return CarbonIntensityTestRunner.env_override(m)


def entity_data(v):
    return CarbonIntensityTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CarbonIntensityTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CarbonIntensityTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CarbonIntensityTestRunner.load_test_control()


def live_client_options():
    return CarbonIntensityTestRunner.live_client_options()


def live_delay_ms():
    return CarbonIntensityTestRunner.live_delay_ms()
