...
class TestIntensityListDirect:

    def test_should_direct_list_intensity_list(self):
        setup = _intensity_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-intensity_list", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_intensity_list(self):
        setup = _intensity_list_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-intensity_list", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _intensity_list_direct_setup(mockres):
