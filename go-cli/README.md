...
boru-driven command-line client **and** interactive REPL for the CarbonIntensity
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/carbon-intensity-cli)
make build

# 2. See usage (words, entities, env vars)
./carbon-intensity-cli --help
...
CARBON_INTENSITY_BASE=https://api.example.com ./carbon-intensity-cli list generation

# 6. No arguments -> interactive REPL
./carbon-intensity-cli
carbon-intensity> list generation
carbon-intensity> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/carbon-intensity-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/carbon-intensity-cli list generation
   ```
...
Run with no arguments to open a REPL (prompt `carbon-intensity>`). Each line is
evaluated as its own boru expression:

```text
$ ./carbon-intensity-cli
carbon-intensity> list generation
carbon-intensity> /help
carbon-intensity> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/carbon-intensity-cli`. |
...
(`./carbon-intensity-cli <expr>`) and interactively (the REPL), and expressions compose the
