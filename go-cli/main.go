// Command CarbonIntensity-cli is a CLI + REPL for the CarbonIntensity SDK,
...
const prompt = "carbon-intensity"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "generation generation_list intensity intensity_factor intensity_list regional regional_intensity regional_intensity_list stat"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CARBON_INTENSITY_APIKEY (api key), CARBON_INTENSITY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CARBON_INTENSITY_APIKEY carries the API key and
	// CARBON_INTENSITY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CARBON_INTENSITY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CARBON_INTENSITY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCarbonIntensitySDK(opts)
