...
[MCP](https://modelcontextprotocol.io) server exposing the CarbonIntensity SDK as
two agent tools — `carbon-intensity_list` and `carbon-intensity_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/carbon-intensity-mcp)
make build

# 2. Provide credentials via the environment
export CARBON_INTENSITY_APIKEY=sk_live_xxx
...
```jsonc
// carbon-intensity_list: first page of records
{ "entity": "generation" }
{ "entity": "generation", "query": { } }

// carbon-intensity_load: one record by id
{ "entity": "generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/carbon-intensity-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user carbon-intensity \
     -- "$PWD"/dist/*/carbon-intensity-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `carbon-intensity_list` and `carbon-intensity_load` tools now appear
   in new sessions. Ask the agent to *"list generation using carbon-intensity"*
   and it calls `carbon-intensity_list` with `{"entity":"generation"}`.
...
```sh
export CARBON_INTENSITY_APIKEY=sk_live_xxx            # API key
export CARBON_INTENSITY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `carbon-intensity_list` tool
...
### Call the `carbon-intensity_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `carbon-intensity_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `carbon-intensity_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CARBON_INTENSITY_APIKEY` | API key sent with every request. |
| `CARBON_INTENSITY_BASE` | Optional override of the API base URL. |
