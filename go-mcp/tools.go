...
type Args struct {
	Entity string         `json:"entity" jsonschema:"generation | generation_list | intensity | intensity_factor | intensity_list | regional | regional_intensity | regional_intensity_list | stat"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CarbonIntensitySDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "carbon-intensity_list",
		Description: "List records from CarbonIntensity. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "carbon-intensity_load",
		Description: "Load a single record from CarbonIntensity. " +
...
func runOp(client *sdk.CarbonIntensitySDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CarbonIntensitySDK, name string) (sdk.CarbonIntensityEntity, error) {
	switch strings.ToLower(name) {
	case "generation":
		return client.Generation(nil), nil
	case "generation_list":
		return client.GenerationList(nil), nil
	case "intensity":
		return client.Intensity(nil), nil
	case "intensity_factor":
		return client.IntensityFactor(nil), nil
	case "intensity_list":
		return client.IntensityList(nil), nil
	case "regional":
		return client.Regional(nil), nil
	case "regional_intensity":
		return client.RegionalIntensity(nil), nil
	case "regional_intensity_list":
		return client.RegionalIntensityList(nil), nil
	case "stat":
